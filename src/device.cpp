...
bool Deltacast::Device::wait_for_incoming_signal(
    int rx_index, std::unique_ptr<VideoMasterVideoInformation>& v_info,
   ...
   while (!stop_is_requested.load())
   {
      ...
      if (api_success && !(status & VHD_CORE_RXSTS_UNLOCKED))
      {
         try
         {
            ...
            auto& sdi_v_info = dynamic_cast<VideoMasterSdiVideoInformation&>(*v_info);
            sdi_v_info.detect_incoming_signal_properties(*_device_handle, rx_index);
         }
...
// bool Deltacast::Device::detect_incoming_signal_properties(
//     int rx_index, std::unique_ptr<VideoMasterVideoInformation>& v_info)
// {
...
//          return {};
//
//       if (!(api_success = ApiSuccess{ VHD_GetBoardProperty(
//                 *handle(), id_to_rx_video_standard_prop.at(rx_index),
//                 (ULONG*)&sdi_v_info.video_standard) }) ||
//           !(api_success = ApiSuccess{ VHD_GetBoardProperty(*handle(),
//                                                            id_to_rx_clock_divisor_prop.at(rx_index),
//                                                            (ULONG*)&sdi_v_info.clock_divisor) })
//                                                            ||
//           !(api_success = ApiSuccess{ VHD_GetBoardProperty(
//                 *handle(), id_to_rx_interface_prop.at(rx_index), (ULONG*)&sdi_v_info.interface)
//                 }))
//       {
...
Deltacast::Device::get_video_information_for_channel(int rx_index)
{
   ...
   auto channel_type_optional = get_channel_type(rx_index, Direction::RX);
...
bool Deltacast::Device::wait_genlock_locked(const std::atomic_bool& stop_is_requested)
{
   while (!stop_is_requested.load())
   {
      ...
      auto  api_success = ApiSuccess{ VHD_GetBoardProperty(*handle(), VHD_SDI_BP_GENLOCK_STATUS,
                                                           &status) };
                                          ...
                                          std::unique_ptr<VideoMasterVideoInformation>& video_info)
{
   ...
   if (genlock_src_property.has_value())
   {
      auto sdi_v_info = dynamic_cast<VideoMasterSdiVideoInformation&>(*video_info);
      if (!(api_success = ApiSuccess{ VHD_SetBoardProperty(*handle(), genlock_src_property.value(),
                                                           id_to_rx_genlock_source.at(
                                                               genlock_source_rx_index)) }) ||
          !(api_success = ApiSuccess{ VHD_SetBoardProperty(
                *handle(), VHD_SDI_BP_GENLOCK_VIDEO_STANDARD, sdi_v_info.video_standard) }) ||
          !(api_success = ApiSuccess{ VHD_SetBoardProperty(*handle(), VHD_SDI_BP_CLOCK_SYSTEM,
                                                           sdi_v_info.clock_divisor) }))
      {
